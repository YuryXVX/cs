...
function benchmarkObjectsIndexOf() {
  ...
  for (const size of sizes) {
    ...
    console.log(`  ✓ Размер ${size.toLocaleString()}: ${(nativeTime / myBinaryTime).toFixed(1)}x ${nativeTime > myBinaryTime ? '🚀' : '🐌'}`);
  }
...
function benchmarkNumbersIndexOf() {
  ...
  for (const size of sizes) {
    ...
    console.log(`  ✓ Размер ${size.toLocaleString()}: ${ratio.toFixed(1)}x ${emoji}`);
  }
...
function generateBenchmarkMD(): string {
  ...
  for (const benchmark of results) {
    ...
    for (const result of benchmark.results) {
      ...
      if (result.speedup) {
        const emoji = result.speedup > 1 ? '🚀' : result.speedup > 0.5 ? '🤝' : '🐌';
  ...
  md += '| Размер | Объекты indexOf | Объекты lastIndexOf | Числа indexOf | Числа lastIndexOf |\n';
  md += '|--------|----------------|--------------------|--------------|------------------|\n';
  ...
  md += '- **Объекты**: бинарный поиск значительно быстрее на больших массивах (>10,000)\n';
  md += '- **Числа**: нативные методы очень оптимизированы, но бинарный поиск выигрывает на огромных массивах\n';
