export function indexOf<T>(col: T[], el: T): number;
export function indexOf<T, K>(
  col: T[],
  el: K,
  selector: (item: T) => K,
): number;
export function indexOf<T, K>(col: T[], el: K, selector?: (el: T) => K) {
  let left = 0;
...
export function lastIndexOf<T>(col: T[], el: T): number;
export function lastIndexOf<T, K>(
  col: T[],
  el: K,
  selector: (item: T) => K,
): number;
export function lastIndexOf<T, K>(col: T[], el: K, selector?: (item: T) => K) {
  let left = 0;
  let right = col.length;
